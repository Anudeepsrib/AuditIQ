...
- All four RBAC roles can authenticate.
"""
...
class TestRBACEnforcement:
    """Test that RBAC restrictions are enforced on /users/ endpoints."""
    ...
    def test_analyst_cannot_list_users(self, client: TestClient):
        """Analyst should be denied access to /users/."""
        login_resp = client.post("/auth/login", json={
    ...
    def test_auditor_cannot_list_users(self, client: TestClient):
        """Auditor should be denied access to /users/."""
        login_resp = client.post("/auth/login", json={
            "username": TEST_USERS["auditor"]["username"],
            "password": TEST_USERS["auditor"]["password"],
        })
        token = login_resp.json()["access_token"]
        response = client.get("/users/", headers=auth_header(token))
        assert response.status_code == 403

    def test_ml_engineer_cannot_list_users(self, client: TestClient):
        """ML Engineer should be denied access to /users/."""
        login_resp = client.post("/auth/login", json={
            "username": TEST_USERS["ml_engineer"]["username"],
            "password": TEST_USERS["ml_engineer"]["password"],
        })
        token = login_resp.json()["access_token"]
        response = client.get("/users/", headers=auth_header(token))
        assert response.status_code == 403
...
class TestAllRolesCanLogin:
    ...
    @pytest.mark.parametrize("role", ["admin", "ml_engineer", "analyst", "auditor"])
