"""
Shared test fixtures for AuditIQ.

Provides:
- In-memory SQLite database with all tables created.
- FastAPI TestClient wired to the test database.
- Pre-created test users for each RBAC role (admin, ml_engineer, analyst, auditor).
...
TEST_DATABASE_URL = "sqlite:///./test_auditiq.db"
...
def setup_database():
    ...
    import os
    if os.path.exists("./test_auditiq.db"):
        os.remove("./test_auditiq.db")
...
def _make_token(role: str) -> str:
    ...
    Args:
        role: One of ``admin``, ``ml_engineer``, ``analyst``, ``auditor``.
...
def ml_engineer_token() -> str:
    """JWT access token for the ml_engineer test user."""
    return _make_token("ml_engineer")
