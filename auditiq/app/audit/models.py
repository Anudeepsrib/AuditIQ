...
class AuditLog(Base):
    ...
    Every write action in AuditIQ (training run, model promotion,
    inference call, user creation, role change) produces an audit record.
    ...
    Attributes:
        ...
        action: Action verb — e.g. ``user_created``, ``model_promoted``.
        resource_type: Type of resource acted upon — e.g. ``user``, ``model``.
